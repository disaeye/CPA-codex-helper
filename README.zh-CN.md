...
对于本周期已有消耗的账号，脚本在原卡片下方新增三行：本周期已用、反推总额度、提前耗尽预警。

![有用量的卡片](./img/card-with-usage.png)

### 未使用的卡片

对于本周期尚未消耗的账号，脚本显示「尚未使用」，并基于同周期窗口下活跃账号的反推额度中位数给出估算总额度。

![未使用的卡片](./img/card-without-usage.png)

### 区块标题聚合

在「Codex 额度」区块标题旁的徽章中展示所有账号的合并视图：已用 / 总额度的 token 与费用、使用百分比（按 30% / 70% 阈值变色）、估算账号数，以及按需显示的耗尽预测时间。

![区块标题聚合](./img/aggregate-summary.png)
